...
bl_info = {
    'name': 'Import Slic3r GCode - not cyclic prints',
    'author': 'Lee Butler',
    'version': (0,1,0),
    'blender': (2, 7, 0),
...
class IMPORT_OT_gcode(bpy.types.Operator):
    ...
    def parse(self, fileName):
        ...
        # self.xOoze = self.xOoze*1.25
        self.xOoze = self.xOoze*3.5
            ...
            for poly in layer:
                if(len(poly)>1):
                    ...
                    for point in poly:
                        ...
                        if (pointNum == 0 or pointNum == len(poly)-1) and layerNum != 0 and layerNum != len(self.layers)-1:
                            if(pointNum ==0):
                                x2,y2,z2 = poly[pointNum+2]
                                direction = np.arctan2(x2-x, y2-y)
                            if pointNum == len(poly)-1:
                                x2,y2,z2 = poly[pointNum-1]
                                direction = np.arctan2(x2-x, y2-y)
                            surface_data = bpy.data.curves.new(layerName, 'SURFACE')
                            surface_data.dimensions = '3D'
                            spline = surface_data.splines.new(type='NURBS')
                            spline.points.add(3)  # already has a default vector
                            spline.points[0].co = (x-self.xOoze*angRad*1.05,y,z+self.ySquash*radius,1)
                            spline.points[1].co = (x+self.xOoze*angRad*1.05,y,z+self.ySquash*radius,1)
                            spline.points[2].co = (x+self.xOoze*angRad*1.05,y,z-self.ySquash*radius,1)
                            spline.points[3].co = (x-self.xOoze*angRad*1.05,y,z-self.ySquash*radius,1)    
...
#                            bpy.ops.object.editmode_toggle()
                            ...
                            # if(pointNum==0):
                            #     pass
                            bpy.ops.transform.rotate(value=-3.14419, axis=(0, 0, 1), constraint_axis=(False, False, True), constraint_orientation='GLOBAL', mirror=False, proportional='DISABLED', proportional_edit_falloff='SMOOTH', proportional_size=1)
                                # bpy.ops.mesh.flip_normals()
                            # else:
                            # if(pointNum==0):
                            bpy.ops.transform.rotate(value=-direction, axis=(0, 0, 1), constraint_axis=(False, False, True), constraint_orientation='GLOBAL', mirror=False, proportional='DISABLED', proportional_edit_falloff='SMOOTH', proportional_size=1)
                            # if(pointNum==len(poly)-1):
                                # bpy.ops.transform.rotate(value=-direction, axis=(0, 0, 1), constraint_axis=(False, False, True), constraint_orientation='GLOBAL', mirror=False, proportional='DISABLED', proportional_edit_falloff='SMOOTH', proportional_size=1)
                                # bpy.ops.mesh.normals_make_consistent()
#                            bpy.ops.mesh.select_all(action = 'DESELECT')
                            bpy.ops.object.mode_set(mode = 'OBJECT') 
                            surface_object.select = False
    #                        bpy.ops.object.mode_set(mode = 'EDIT') 
...
def menu_func(self, context):
    self.layout.operator(IMPORT_OT_gcode.bl_idname, text="Slic3r GCode noncyclic(.gcode)", icon='PLUGIN')
